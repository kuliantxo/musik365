...
function GenresStationsCtrl($scope, $http, mySharedService, $routeParams) {
	...
	var url = 'http://www.live365.com/cgi-bin/directory.cgi?site=xml&access=PUBLIC&rows=5&only=P&genre='+escape($routeParams.genreID);

	$scope.class = 'loading';
	$scope.currentStation = mySharedService.station.STATION_BROADCASTER;
	$scope.currentStatus = mySharedService.station.status;

	$scope.genreTitle = $routeParams.genreID;
