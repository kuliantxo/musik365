...
function GenresStationsCtrl($scope, $http, mySharedService, $routeParams) {
	...
	var url = 'http://www.live365.com/cgi-bin/directory.cgi?site=xml&access=PUBLIC&rows=5&only=P&genre='+mySharedService.genres.ext[$routeParams.genreID],
		currentStation = mySharedService.station.STATION_BROADCASTER,
		currentStatus = mySharedService.station.status;
console.log(url);
	$scope.class = 'loading';

	$scope.genreTitle = mySharedService.genres.ext[$routeParams.genreID];
